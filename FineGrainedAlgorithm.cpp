...
//locks for achieving successful fine grained algorithm
pthread_mutex_t *mutexlock,lock_var;
...
//function to colour the graph((fine - grained)), as several threads invoke the function
...
void colour_graph_segment(int id, Graph_colouring* graph)
{
	...
	for(int ver : external_vertices)
	{
		...
		//lock all the neighbours
		for(auto neigh : neighbours)
		pthread_mutex_lock(&mutexlock[neigh]);
		...
		//unlock all the neighbours
		for(auto neigh : neighbours)
		pthread_mutex_unlock(&mutexlock[neigh]);
	}
...
int main()
{
	...
	if(file.is_open())
	{
		...
		//assigning the locks
		mutexlock = new pthread_mutex_t[number_of_vertices];
		...
		for(int i = 0 ; i < number_of_vertices ; i++)
		{
			//initialising the ith mutex lock
		...
		//destroying the initialised mutex locks
		for(int i = 0 ; i < number_of_vertices ; i++)
		...
		outfile<<"Fine-Grained Algorithm:"<<endl<<endl;
		...
		outfile<<"timetaken in milliseconds= "<<(end.tv_sec-start.tv_sec)*1e3+(end.tv_usec-start.tv_usec)/1e3<<"milli seconds"<<endl<<endl;		//printing the stored string to the output file
		
		
		
		outfile<<"Assigned colours:"<<endl;
	
		//using a for loop, the data is stored in the string stream object
		for(int i = 0 ; i < number_of_vertices ; i++)
		{
			outfile<<"vertex-"<<i+1<<" colour-"<<colours[i]<<endl;
		}

		
		//freeing the graph pointer
		delete graph;
		//deleting the mutex locks
		delete[] mutexlock;
	}
